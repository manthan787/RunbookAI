...
print_next_steps() {
    ...
    echo ""
    echo -e "${YELLOW}${BOLD}>>> Restart your shell to use runbook:${NC}"
    echo ""
    echo -e "    ${CYAN}exec \$SHELL -l${NC}"
    echo ""
    echo -e "${BOLD}Then follow these steps:${NC}"
    echo ""
    echo "  1. Initialize configuration:"
    echo -e "     ${CYAN}runbook init${NC}"
    echo ""
    echo "  2. Ask your first question:"
    echo -e "     ${CYAN}runbook ask \"What EC2 instances are running?\"${NC}"
    echo ""
    echo -e "${BOLD}Documentation:${NC} https://userunbook.ai"
